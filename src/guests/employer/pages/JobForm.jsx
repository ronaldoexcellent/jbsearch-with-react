...
import WorkExperience from "../questions/WorkExperience";
...
const FormBuilder = () => {
  ...
  const [load, setLoad] = useState('Taking you to jobs!');

  const handleSubmit = () => {
    // if (!validate()) return;
    toast.success('Job Post Created Successfully!');
    setTimeout(() => setLoad(toast.loading(load)), 3000);
    setTimeout(() => {
      setLoad(l => toast.remove(l));
      nav("/employer/jobs");
    }, 6500);
  };
  ...
  const steps = [
    <PersonalInfo i={index} p={prev} n={next} handleSubmit={handleSubmit} />,
    <WorkExperience i={index} p={prev} n={next} handleSubmit={handleSubmit} />,
    <Education i={index} p={prev} n={next} handleSubmit={handleSubmit} />,
    <Summary i={index} p={prev} n={next} handleSubmit={handleSubmit} />
  ];

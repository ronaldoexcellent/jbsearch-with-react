...
function MenuBtn({ reverseColor, adjust, func }) {
  ...
  return (
    <button
      ...
      onClick={ func }
    >
      <span className={`p-4 transition text-cemter duration-300 focus:outline-none font-extrabold hover:rotate-180 ${rC}`}>☰</span>
    </button>
  )
}
...
function Sidebar({ menuItems }) {
  ...
  const navigate = (target) => {
    ...
    const timer = setTimeout(() => { stopProgress(); toast.remove(load) }, 3000);
